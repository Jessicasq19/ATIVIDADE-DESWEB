<script setup lang="ts">
    ...
    //para lidar com promissespodemos usar o try/catch
    const submitLogin = async()=>{ // async = assincrona
        console.log("trying to login with credentials below:", credentials);
        try{
            //await = espere (espere o signIn)
            await signIn(credentials, {redirect: false});
            navigateTo('/home');
        } catch(error){
            //catch = se não (se não fizer o signIn, cai no catch)
            console.log("Error when trying to login: ", error);
        }
    
    /*Também podemos lidar com promisses usando then/catch
    const submitLogin = async()=>{ // async = assincrona
        console.log("trying to login with credentials below:", credentials);
        signIn(credentials, {redirect: false}).then(()=>{
            console.log("successflly logged!");
            navigateTo('/home');
        })
        .catch((error)=>{
            catch = se não (se não fizer o signIn, cai no catch)
         ...
         <section class="login_panel">
            <div class="login_content flex_center">
                ...
                <form class="login_form" v-on:submit.prevent="submitLogin">
                    ...
                    <!--<button @click="submitLogin" class="customButton">ENTRAR</button>-->
